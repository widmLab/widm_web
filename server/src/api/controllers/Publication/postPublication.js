...
exports.postPublication = function(req,res){
   ...
   if(name == undefined | date == undefined | tid == undefined){
     res.status(400).json({status:"Bad Request",data:'Wrong format of post body ( need name,date,tid )'});
      ...
      qs = "Select PID+1 available_id from Publication t where not exists(select * from Publication where PID = t.PID + 1) order by PID Limit 1"
      db
      .query(qs)
      .then(data=>{
        console.log(data[0])
      ...
      .then(()=>{
        return new Promise(function(resolve,reject){
            db
            .query(`Insert into Publication(PID,Publication_Name,Date,TID) VALUES (${id},\"${name}\",\"${date}\",${tid})`)
            .then(data => {
                console.log('~')
